...
class GoalsCard(ft.Card):
    def __init__(self, goal: Goal, on_edit, on_delete, on_progress_update=None):
        ...
        if self.goal_class == "reductual":
            self.progress_btn = ft.IconButton(ft.Icons.REMOVE.value, tooltip="Reducir progreso", on_click=self._handle_reduce_progress)
        else:
            self.progress_btn = ft.IconButton(ft.Icons.ADD.value, tooltip="Sumar progreso", on_click=self._handle_add_progress)
